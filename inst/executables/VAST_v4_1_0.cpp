...
Type dPoisGam( Type x, Type shape, Type scale, Type intensity, vector<Type> &diag_z, int maxsum=50, int minsum=1, int give_log=0 ){
  ...
  Type W = 0;
  Type log_w_j;
  //Type pos_penalty; // no longer used now that line in loop below is commented out
...
Type objective_function<Type>::operator() ()
{
  ...
  //DATA_INTEGER(n_j);         // Number of static covariates (no longer used)
  DATA_INTEGER(n_p);         // Number of dynamic covariates
  //DATA_INTEGER(n_k);          // Number of catchability variables (no longer used)
