  ...
  for(i=0; i<n_i; i++){
    if( !isNA(b_i(i)) ){
      ...
      if(ObsModel_ez(e_i(i),0)==9){
        ...
        logdBinPois(0) = logspace_add( log(1.0-R1_i(i)), dpois(Type(0.0), R2_i(i), true) + log(R1_i(i)) ); //  Pr[X=0] = 1-phi + Pois(X=0)*phi
      ...
      if(ObsModel_ez(e_i(i),0)==11){
        if( b_i(i)==0 ){
          //LogProb2_i(i) = log( (1-R1_i(i)) + dpois(Type(0.0), R2_i(i), false)*R1_i(i) ); //  Pr[X=0] = 1-phi + Pois(X=0)*phi
          LogProb2_i(i) = logspace_add( log(1.0-R1_i(i)), dpois(Type(0.0),R2_i(i)*exp(SigmaM(e_i(i),0)*delta_i(i)-0.5*pow(SigmaM(e_i(i),0),2.0)),true)+log(R1_i(i)) ); //  Pr[X=0] = 1-phi + Pois(X=0)*phi
        }else{
      ...
      if(ObsModel_ez(e_i(i),0)==13){
        if( b_i(i)==0 ){
          ...
          LogProb2_i(i) = logspace_sub( log(Type(1.0)), dpois(Type(0.0), R1_i(i), true) );
        }
        // Simulate new values when using obj.simulate()
        SIMULATE{
          b_i(i) = rpois( R1_i(i) );
