...
matrix<Type> create_loadings_general( vector<Type> L_val, int n_rows, int n_f, Type zerosum_penalty, Type trace_sum_penalty, Type &jnll_pointer ){
  if( n_f == -2 ){
    ...
    //  return L_rc;
    //}else{
    matrix<Type> L_rc = create_loadings_covariance( L_val, n_rows, n_f, zerosum_penalty, trace_sum_penalty, jnll_pointer );
    return L_rc;
    //}
  }
}
...
Type objective_function<Type>::operator() ()
{
  ...
  // Object used by TMBhelper::oneStepPredict_deltaMethod(.)
  DATA_VECTOR_INDICATOR(keep, b_i);
  ...
  matrix<Type> L_omega1_cf = create_loadings_general( L_omega1_z, n_c, FieldConfig(0,0), Type(0.0), Type(0.0), jnll_comp(19) );
  matrix<Type> L_omega2_cf = create_loadings_general( L_omega2_z, n_c, FieldConfig(0,1), Type(0.0), Type(0.0), jnll_comp(19) );
  matrix<Type> L_epsilon1_cf = create_loadings_general( L_epsilon1_z, n_c, FieldConfig(1,0), Type(0.0), Type(0.0), jnll_comp(19) );
  matrix<Type> L_epsilon2_cf = create_loadings_general( L_epsilon2_z, n_c, FieldConfig(1,1), Type(0.0), Type(0.0), jnll_comp(19) );
  matrix<Type> L_beta1_cf = create_loadings_general( L_beta1_z, n_c, FieldConfig(2,0), Type(0.0), Type(0.0), jnll_comp(19) );
  matrix<Type> L_beta2_cf = create_loadings_general( L_beta2_z, n_c, FieldConfig(2,1), Type(0.0), Type(0.0), jnll_comp(19) );
  matrix<Type> Ltime_epsilon1_tf = create_loadings_general( Ltime_epsilon1_z, n_t, FieldConfig(3,0), zerosum_penalty(0,0), trace_sum_penalty(0), jnll_comp(19) );
  matrix<Type> Ltime_epsilon2_tf = create_loadings_general( Ltime_epsilon2_z, n_t, FieldConfig(3,1), zerosum_penalty(0,0), trace_sum_penalty(0), jnll_comp(19) );
  matrix<Type> L_eta1_cf = create_loadings_general( L_eta1_z, n_c, OverdispersionConfig(0), Type(0.0), Type(0.0), jnll_comp(19) );
  matrix<Type> L_eta2_cf = create_loadings_general( L_eta2_z, n_c, OverdispersionConfig(1), Type(0.0), Type(0.0), jnll_comp(19) );
  ...
  for(i=0; i<n_i; i++){
    if( !isNA(b_i(i)) ){
      ...
      if( (ObsModel_ez(e_i(i),1)==1) | (ObsModel_ez(e_i(i),1)==4) ){
        ...
        // Calulate in logspace to prevent numerical over/under-flow
        log_R1_i(i) = logspace_sub( Type(0.0), -1*a_i(i)*tmp_calc1 );
      ...
      if( ObsModel_ez(e_i(i),1)==2 ){
        ...
        log_one_minus_R1_i(i) = logspace_sub(Type(0.0), log(a_i(i)) + P1_iz(i,0) );
      ...
      // Aggregate
      jnll_comp(10) -= keep(i) * LogProb1_i(i) * (Type(1.0)-PredTF_i(i));
      jnll_comp(11) -= keep(i) * LogProb2_i(i) * (Type(1.0)-PredTF_i(i));
      // CDF aggregator
      jnll_comp(18) -= keep.cdf_lower(i) * log( squeeze(cdf_for_single_obs) );
      jnll_comp(18) -= keep.cdf_upper(i) * log( 1.0 - squeeze(cdf_for_single_obs) );
      // Predictive jnll aggregator
      pred_jnll -= (LogProb1_i(i) + LogProb2_i(i)) * PredTF_i(i);
    }
  }
  REPORT( diag_iz );
