...
knitr::opts_chunk$set(cache=TRUE, cache.path="cache_multi/")
```


```{r cache=TRUE, set_options, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r cache=TRUE, wrap-hook, echo=FALSE}
...
```{r cache=TRUE, load_packages, message=FALSE}
devtools::install_github("james-thorson/VAST") 
```

We also install `FishData`, which is used to download data for our example
```{r cache=TRUE, load_other_packages, message=FALSE}
devtools::install_github("james-thorson/FishData")
```

Next load libraries.
```{r cache=TRUE, load_libraries, warning=FALSE, message=FALSE}
...
If you have further questions after reading this tutorial, please explore the [GitHub repo](https://github.com/james-thorson/VAST/#description) mainpage, wiki, and glossary.  Also please explore the R help files, e.g., e.g., `?Data_Fn` for explanation of data inputs, or `?Param_Fn` for explanation of parameters.
...
```{r cache=TRUE, citation, tidy=TRUE, width=70, width.cutoff=70}
citation("VAST")
```
...
```{r cache=TRUE, cache=TRUE, define_options, tidy=TRUE}
...
```{r cache=TRUE, define_strata, tidy=TRUE, linewidth=50}
...
```{r cache=TRUE, define_region, tidy=FALSE}
...
```{r cache=TRUE, make_dir, message=FALSE, warning=FALSE}
DateFile = paste0(getwd(),'/Multi-species/')
  dir.create(DateFile)
```

I also like to save all settings for later reference, although this is not necessary.
```{r, cache=TRUE, tidy=TRUE, linewidth=50}
Record = list("Version"=Version,"Method"=Method,"grid_size_km"=grid_size_km,"n_x"=n_x,"FieldConfig"=FieldConfig,"RhoConfig"=RhoConfig,"OverdispersionConfig"=OverdispersionConfig,"ObsModel"=ObsModel,"Kmeans_Config"=Kmeans_Config,"Region"=Region,"Species_set"=Species_set,"strata.limits"=strata.limits)
...
```{r cache=TRUE, load_data, echo=TRUE, message=FALSE, tidy=TRUE}
...
```{r cache=TRUE, show_data_head, results="asis", echo=FALSE}
pander::pandoc.table( head(Data_Geostat), digits=3 )
```

... and tail
```{r cache=TRUE, show_data_tail, results="asis", echo=FALSE}
...
```{r cache=TRUE, extrapolation_grid, message=FALSE, tidy=TRUE, linewidth=60}
...
```{r cache=TRUE, spatial_information, message=FALSE, warning=FALSE, tidy=TRUE, linewidth=60}
...
```{r cache=TRUE, build_data, message=FALSE, tidy=TRUE, linewidth=60, results="hide"}
...
```{r cache=TRUE, build_object, message=FALSE, results="hide", tidy=TRUE}
...
```{r cache=TRUE, explore_data, results="hide", tidy=TRUE, message=FALSE, warning=FALSE}
...
```{r cache=TRUE, print_results, results="asis"}
...
```{r cache=TRUE, diagnostics_encounter_prob, results="hide", eval=TRUE, tidy=TRUE, linewidth=50}
...
```{r cache=TRUE, plot_QQ, eval=TRUE, tidy=TRUE, linewidth=50, message=FALSE, warning=FALSE}
...
```{r cache=TRUE, plot_pearson_resid, message=FALSE, warning=FALSE, tidy=TRUE, linewidth=50}
...
```{r cache=TRUE, plot_aniso, message=FALSE, results="hide", tidy=TRUE}
...
```{r cache=TRUE, plot_covariance, tidy=TRUE}
...
```{r cache=TRUE, plot_density, message=FALSE, warning=FALSE, tidy=TRUE, linewidth=50}
...
```{r cache=TRUE, plot_index, message=FALSE, tidy=TRUE, linewidth=50, results="asis"}
...
```{r cache=TRUE, plot_range, message=FALSE, tidy=TRUE, linewidth=50}
...
```{r cache=TRUE, plot_overdispersion, message=TRUE, tidy=TRUE}
...
```{r cache=TRUE, plot_factors, message=FALSE, tidy=TRUE, results="hide", warning=FALSE}
