...
Next load libraries.
```{r cache=TRUE, load_libraries, message=FALSE, warning=FALSE}
...
DateFile = paste0(getwd(),'/Single-species/')
  dir.create(DateFile)
```
...
```{r cache=TRUE, load_data, message=FALSE, tidy=TRUE, linewidth=50}
...
```{r cache=TRUE, build_data, message=FALSE, tidy=TRUE, linewidth=60}
...
```{r cache=TRUE, save_results, linewidth=60}
Report = Obj$report()
...
```{r cache=TRUE, calc_density_dataframe, message=FALSE, warning=FALSE, tidy=TRUE, linewidth=50}
...
```{r cache=TRUE, show_density_head, results="asis", echo=FALSE}
