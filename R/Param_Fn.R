...
function( Version, DataList, RhoConfig=c("Beta1"=0,"Beta2"=0,"Epsilon1"=0,"Epsilon2"=0) ){
  ...
  if(Version%in%c("VAST_v7_0_0")){
    Return = list("ln_H_input"=c(0,0), "Chi_fr"=rarray(dim=c(max(DataList$FieldConfig[2],1),DataList$VamConfig[2])), "Psi_fr"=rarray(dim=c(max(DataList$FieldConfig[2],1),DataList$VamConfig[2])), "beta1_tf"=NA, "gamma1_ctp"=array(0,dim=c(DataList$n_c,DataList$n_t,DataList$n_p)), "lambda1_k"=rep(0,ncol(DataList$Q_ik)), "L1_z"=NA, "L_omega1_z"=NA, "L_epsilon1_z"=NA, "L_beta1_z"=NA, "logkappa1"=log(0.9), "Beta_mean1_f"=NA, "Beta_rho1_f"=NA, "Epsilon_rho1_f"=NA, "log_sigmaXi1_cp"=array(0,dim=c(DataList$n_c,DataList$n_p)), "log_sigmaratio1_z"=rep(0,ncol(DataList$t_iz)), "eta1_vf"=NA, "Xiinput1_scp"=array(0,dim=c(DataList$n_s,DataList$n_c,DataList$n_p)), "Omegainput1_sf"=NA, "Epsiloninput1_sft"=NA, "beta2_tf"=NA, "gamma2_ctp"=array(0,dim=c(DataList$n_c,DataList$n_t,DataList$n_p)), "lambda2_k"=rep(0,ncol(DataList$Q_ik)), "L2_z"=NA, "L_omega2_z"=NA, "L_epsilon2_z"=NA, "L_beta2_z"=NA, "logkappa2"=log(0.9), "Beta_mean2_f"=NA, "Beta_rho2_f"=NA, "Epsilon_rho2_f"=NA, "log_sigmaXi2_cp"=array(0,dim=c(DataList$n_c,DataList$n_p)), "log_sigmaratio2_z"=rep(0,ncol(DataList$t_iz)), "logSigmaM"=rep(1,DataList$n_e)%o%c(log(5),log(2),log(1)), "delta_i"=rnorm(n=ifelse(any(DataList$ObsModel_ez[,1]%in%c(11,14)),DataList$n_i,1),sd=0.1), "eta2_vf"=NA, "Xiinput2_scp"=rarray(0,dim=c(DataList$n_s,DataList$n_c,DataList$n_p)), "Omegainput2_sf"=NA, "Epsiloninput2_sft"=NA )
  }
  ...
  if( "L_beta1_z" %in% names(Return)) Return = Add_factor( List=Return, n_c=DataList$n_c, n_f=DataList$FieldConfig[5], n_i=DataList$n_t, list_names=c("L_beta1_z","beta1_tf"), sd=0 )
  ...
  if( "L_beta2_z" %in% names(Return)) Return = Add_factor( List=Return, n_c=DataList$n_c, n_f=DataList$FieldConfig[6], n_i=DataList$n_t, list_names=c("L_beta2_z","beta2_tf"), sd=0 )
  ...
  if( "Beta_rho1_f" %in% names(Return)) Return[["Beta_rho1_f"]] = rep(0, ncol(Return[["beta1_tf"]]))
  if( "Beta_rho2_f" %in% names(Return)) Return[["Beta_rho2_f"]] = rep(0, ncol(Return[["beta2_tf"]]))

  # Mean for intercepts (must be ZERO by default)
  if( "Beta_mean1_f" %in% names(Return)) Return[["Beta_mean1_f"]] = rep(0, ncol(Return[["beta1_tf"]]))
  if( "Beta_mean2_f" %in% names(Return)) Return[["Beta_mean2_f"]] = rep(0, ncol(Return[["beta2_tf"]]))

  # Informative starting values < 7.0.0
  ...
  if( all(c("beta1_tf","beta2_tf") %in% names(Return)) ){
    Return[["beta1_tf"]] = tmpfn( Return[["beta1_tf"]] )
    Return[["beta2_tf"]] = tmpfn( Return[["beta2_tf"]] )
  }
