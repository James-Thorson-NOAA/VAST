...
function( Version, DataList, RhoConfig=c("Beta1"=0,"Beta2"=0,"Epsilon1"=0,"Epsilon2"=0) ){
  ...
  if( "L_beta1_z" %in% names(Return)){
    Return = Add_factor( List=Return, n_c=DataList$n_c, n_f=DataList$FieldConfig[5], n_i=DataList$n_t, list_names=c("L_beta1_z","beta1_ft"), sd=0 )
  ...
  if( "L_beta2_z" %in% names(Return)){
    Return = Add_factor( List=Return, n_c=DataList$n_c, n_f=DataList$FieldConfig[6], n_i=DataList$n_t, list_names=c("L_beta2_z","beta2_ft"), sd=0 )
  ...
  if( "Beta_mean1_f" %in% names(Return)) Return[["Beta_mean1_f"]] = rep(0, nrow(Return[["beta1_ft"]]))
  if( "Beta_mean2_f" %in% names(Return)) Return[["Beta_mean2_f"]] = rep(0, nrow(Return[["beta2_ft"]]))
  ...
  if( Use_informative_starts==TRUE ){
    ...
    # Deal with problems
    if(length(Return[["beta1_ct"]])==1 && is.na(Return[["beta1_ct"]])){
      Return[["beta1_ct"]] = array(0, dim=c(DataList$n_c,DataList$n_t))
    }
    if(length(Return[["beta2_ct"]])==1 && is.na(Return[["beta2_ct"]])){
      Return[["beta2_ct"]] = array(0, dim=c(DataList$n_c,DataList$n_t))
    }
  }
