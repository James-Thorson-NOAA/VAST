  ...
  if( !is.null(figname) ){
    ...
    save_fig( file=paste0(figname,"--Analytic"), width=Dim[2]*4+1, height=Dim[1]*4 )
      ...
      for(i in which(plotTF) ){
        ...
        plot_cov( Cov=convert(Cov_cc)[species_order,species_order], names=list(names_set[species_order],NA)[[ifelse(i==1|i==3|Dim[2]==1,1,2)]], names2=list(1:nrow(Cov_cc),NA)[[ifelse(i==1|i==2,1,2)]], digits=1, font=2 )
