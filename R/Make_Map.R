Make_Map <-
function( Version, TmbData, CovConfig=TRUE, DynCovConfig=TRUE, Q_Config=TRUE, RhoConfig=c("Beta1"=0,"Beta2"=0,"Epsilon1"=0,"Epsilon2"=0), Aniso=TRUE){
  # Local functions
  ...
  # Beta1 -- Fixed
  if( RhoConfig["Beta1"]==0 && Version%in%c("comp_index_v1a")){
    Map[["Beta_mean1"]] = factor( NA )
    Map[["Beta_rho1"]] = factor( NA )
    Map[["logsigmaB1"]] = factor( NA )
  }
  # Beta1 -- White-noise
  if( RhoConfig["Beta1"]==1 && Version%in%c("comp_index_v1a")){
    Map[["Beta_rho1"]] = factor( NA )
  }
  # Beta1 -- Random-walk
  if( RhoConfig["Beta1"]==2 && Version%in%c("comp_index_v1a")){
    Map[["Beta_mean1"]] = factor( NA )
    Map[["Beta_rho1"]] = factor( NA )
  }
  # Beta1 -- Constant
  if( RhoConfig["Beta1"]==3 && Version%in%c("comp_index_v1a")){
    Map[["Beta_mean1"]] = factor( NA )
    Map[["Beta_rho1"]] = factor( NA )
    Map[["logsigmaB1"]] = factor( NA )
  ...
  # Beta2 -- Fixed
  if( RhoConfig["Beta2"]==0 && Version%in%c("comp_index_v1a")){
    Map[["Beta_mean2"]] = factor( NA )
    Map[["Beta_rho2"]] = factor( NA )
    Map[["logsigmaB2"]] = factor( NA )
  }
  # Beta2 -- White-noise
  if( RhoConfig["Beta2"]==1 && Version%in%c("comp_index_v1a")){
    Map[["Beta_rho2"]] = factor( NA )
  }
  # Beta2 -- Random-walk
  if( RhoConfig["Beta2"]==2 && Version%in%c("comp_index_v1a")){
    Map[["Beta_mean2"]] = factor( NA )
    Map[["Beta_rho2"]] = factor( NA )
  }
  # Beta2 -- Constant
  if( RhoConfig["Beta2"]==3 && Version%in%c("comp_index_v1a")){
    Map[["Beta_mean2"]] = factor( NA )
    Map[["Beta_rho2"]] = factor( NA )
    Map[["logsigmaB2"]] = factor( NA )
  ...
  if( RhoConfig["Epsilon1"]%in%c(0,1,2) && Version%in%c("comp_index_v1a")){
    Map[["Epsilon_rho1"]] = factor( NA )
  }
  # Epsilon2 -- Fixed OR White-noise OR Random walk
  if( RhoConfig["Epsilon2"]%in%c(0,1,2) && Version%in%c("comp_index_v1a")){
